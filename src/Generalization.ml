...
let generalize (uenv : uenv) (env : env) (generation : generation) : uenv * env * variable list =
  (* Initialize *)
  let young = Env.get_young env in
  let uenv = ref uenv in
  let env = ref env in
...
let exit (uenv : uenv) (env : env) (roots : variable list) : uenv * env * scheme list =
    ...
    List.fold_left
      begin
        fun uenv var ->
          ...
          let repr = UEnv.repr var uenv in
          let structure_copy =
            Option.map (Structure.map copy) repr.structure
          in
          let copy_repr =
